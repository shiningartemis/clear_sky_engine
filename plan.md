...
- [x] 阶段 1：工程与本地运行——工具链、项目骨架、数据库、前端空壳、本地安全启动器、检查和打包。
- [x] 阶段 2：AI Provider——Provider/Model 管理、OpenAI-compatible、DeepSeek、流式协议、脱敏及真实 API 验证。
- [x] 阶段 3：世界、角色与地图——世界/分支、角色、属性、位置、时间、地图与不依赖 AI 的完整配置体验。
...
- [x] 使用 respx 先覆盖普通响应、SSE 分块/空行、Tool Calls、JSON Output、usage、取消和错误映射的失败测试。
- [x] 实现 `OpenAICompatibleProvider` 普通调用与流式解析，并保持两条路径的公共语义一致。
- [x] 仅在尚未产生有效输出时对临时网络错误有限重试；重试感知取消，不引入额外重试依赖。
